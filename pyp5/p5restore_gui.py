...
class RestoreGUI(tk.Tk):
    def __init__(self):
        ...
        self.label_log = tk.Label(frame_04, text='Log Output:', background='blue', foreground='white')
        self.label_log.grid()

        frame_14 = tk.Frame(frame_main)
        frame_14.grid(row=1, column=4, rowspan=5, padx=(10, 0), sticky=tk.NSEW)
        self.text_log_output = ScrolledText(frame_14, highlightthickness=0, height=41, width=80, background='black',
                                            foreground='white', wrap=tk.WORD)
        ...
        self.check_dryrun = tk.Checkbutton(frame_32, text='Dry run', variable=self.check_dryrun_state)
        ...
        self.check_mail = tk.Checkbutton(frame_33, text='Send log', variable=self.check_mail_state)
        self.check_mail.grid(pady=(0, 5))

        # --- CONFIG
        frame_40 = tk.Frame(frame_main)
        frame_40.grid(row=4, column=0, columnspan=4, sticky=tk.NSEW)
        self.label_config = tk.Label(frame_40, text='Config:', background='orange', foreground='white')
        ...
        self.label_file = tk.Label(frame_60, text=f'Current file: {self.selected_file}', foreground='grey')
        ...
        self.label_bug_report = tk.Label(frame_64, text='Bug Report', foreground='grey', cursor='hand')
    ...
    def config_load(self):
        ...
        try:
            ...
            self.text_log_output.insert(tk.END, f'[{get_time()}] ERROR: no valid config file found.')
    ...
    def config_change(self, selected_index):
        index = 0
        if selected_index == 'Archiv_p5':
            index = '10003'
        elif selected_index == 'Axle':
            index = '10004'
        config_modified = "".join([s for s in self.text_config.get(1.0, tk.END).strip().splitlines(True)
                                   if s.strip("\r\n").strip()])
        self.config_parser.read_string(config_modified)
        self.config_parser['RESTORE']['archive_id'] = index
        self.config_save()
    ...
    def open_file(self):
        filetypes = (('ALE files', '*.ale'), ('AAF files', '*.aaf'), ('EDL files', '*.edl'))
        self.selected_file = fd.askopenfilename(parent=self, initialdir=f'{os.path.expanduser("~")}',
                                                filetypes=filetypes)
        ...
        if 'ERROR' in self.entries:
            self.text_log_output.insert(tk.INSERT, f"[{get_time()}] '{os.path.basename(self.selected_file)}': "
                                                   f"{': '.join(self.entries)}\n")
            return
        ...
        self.update_labels()
        self.text_log_output.insert(tk.INSERT, f'[{get_time()}] Opened file: '
                                               f'"{os.path.basename(self.selected_file)}"\n')
        self.text_log_output.insert(tk.INSERT, f'[{get_time()}] Found {self.sum_entries} entries in file.\n')
    ...
    def restore(self):
        ...
        if not restore_selection:
            self.text_log_output.insert(tk.END, f'[{get_time()}] ERROR: Could not create restore selection.\n')
            return
        ...
        for item in self.list_entries.get(0, tk.END):
            ...
            if result == '0' or not result:
                self.text_log_output.insert(tk.END, f'[{get_time()}] {item.strip()} not found.\n')
            else:
                self.text_log_output.insert(tk.END, f'[{get_time()}] {item.strip()}: {result}\n')
            self.text_log_output.yview(tk.END)

        entries = pyp5.check_output(nsdchat + ['-c', 'RestoreSelection', restore_selection,
                                               'entries']).decode('utf-8').strip('\n')
        ...
        if not volumes:
            self.text_log_output.insert(tk.END, f'[{get_time()}] ERROR: No volumes found for {restore_selection}.\n')
            return
        ...
        if self.check_mail_state.get():
            ...
            postbote.send(list(self.config_parser.get('NOTIFICATION', 'email').split(',')),
                          f'Log Output: {os.path.basename(self.selected_file)}', content)
    ...
    def save_logfile(self):
        ...
        with open(f"{os.path.expanduser('~')}/pyp5_restore-{dt.datetime.now().strftime('%Y%m%d-%H%M%S')}.log", 'w') \
                as log_file:
            log_file.write(output)
            mb.showinfo('Information', f'Logfile saved to {os.path.expanduser("~")}', parent=self)
...
class BugReport(tk.Tk):
    def __init__(self, parent):
        ...
        self.bug_report_text = tk.Text(frame_bug_report, highlightthickness=0, height=10, width=80)
        ...
        self.text_log_output = tk.Text(frame_bug_report, highlightthickness=0, height=5, width=80, wrap=tk.WORD)
        self.text_log_output.configure(font=('Andale Mono', 14))

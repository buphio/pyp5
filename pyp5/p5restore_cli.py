...
def read_conf():
    ...
    try:
        ...
        nsdchat = [config.get('GENERAL', 'nsdchat')] + config.get('GENERAL', 'awsock').split()
...
def main(current_dir, argv=None) -> int:
    ...
    parser = argparse.ArgumentParser(description='Restores files supplied via .aaf or .ale')
    parser.add_argument('-d', '--dry', action='store_true', help='does not submit restore selection')
    parser.add_argument('-m', '--mail', action='store_true', help='only send testmail (no execution of script)')
    args = parser.parse_args(argv)

    # --- create logger
    app_logger = init_logger('APP', logging.INFO, f'{current_dir}/logs/pyp5.log')
    ...
    if args.mail:
        print(postbote.send(mail, 'Test-Mail', 'Mail notification is working.'))
        return 0

    ale_files = [f for f in glob.glob(f"{current_dir}/restore/*.[aA][lL][eE]")]
    aaf_files = [f for f in glob.glob(f"{current_dir}/restore/*.[aA][aA][fF]")]
    ...
    for file in files:
        ...
        if search_items[0] == 'ERROR':
            app_logger.error(f'"{file}": {search_items[1]}')
            os.system(f'mv {file} {current_dir}/failed/')
            continue

        restore_selection = pyp5.new_restore_selection(nsdchat).strip("\n")
        if not restore_selection:
            app_logger.critical(f"Couldn't create restore selection: {file}")
            app_logger.info(pyp5.get_error(nsdchat))
            continue

        file_logger = init_logger('FILE', logging.INFO, f'{current_dir}/logs/{os.path.basename(file)}.log')
        file_logger.info(f"Created RestoreSelection: {restore_selection}")

        # --- search for entries and add to restore selection
        for item in search_items:
            result = pyp5.find_entry(nsdchat, restore_selection, archive_id, item).strip("\n")
            print(f'{item}: {result}')
            if result == '0' or not result:
                file_logger.warning(f"{item} not found in archive.")
        ...
        if not entries or entries == '0':
            file_logger.critical(f"No entries in Restore Selection.")
            continue

        file_logger.info(f"Added {entries} entries to Restore Selection")
        ...
        for volume in sorted(volumes_list):
            ...
            file_logger.info(f"{volume}: {label}")
        ...
        if not args.dry:
            ...
            if not job_id:
                file_logger.critical(f"Could not submit restore selection!")
                postbote.send(mail, 'ERROR', f'Could not submit restore selection of "{file}".')
                continue

            file_logger.info(f"Created restore job with ID {job_id}")
